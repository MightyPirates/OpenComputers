...
local function simpleLoad(name)
    local conf, reason = loadConfig()
    if not conf then
        return nil, reason
    end
    return load(name, conf[name])
end

rc.simpleLoad = simpleLoad
rc.load = load
...
function rc.get(name)
    if not rc.loaded[name] then
        local ok, err = simpleLoad(name)
        if ok then
            return rc.loaded[name]
        else
            return nil, err
...
rc.rawRunCommand = rawRunCommand
...
local function raw_run(conf, name, cmd, args, ...)
    local result, what = load(name, args)
    if result then
        if not cmd then
            return false, "this command is not found"
        elseif type(result[cmd]) == "function" then
            result, what = xpcall(result[cmd], debug.traceback, ...)
            if result then
                return true
...
local function run(name, cmd, ...)
    ...
    return raw_run(conf, name, cmd, conf[name], ...)
end

rc.run = run

function rc.list(name)
    if name then
        return simpleLoad(name)
    else
        local tbl = {}
        for data in fs.list("/etc/rc.d") do
            local full_path = fs.concat("/etc/rc.d", data)
            if not fs.isDirectory(full_path) then
                local exception = ".lua"
                if unicode.sub(data, unicode.len(data) - 3) == exception then
                    table.insert(tbl, unicode.sub(data, 1, unicode.len(data) - 4))
                end
            end
        end
        return tbl
...
function rc.enabled()
    local conf, err = loadConfig()
    if not conf then return nil, err end
    return conf.enabled
end

rc.loadConfig = loadConfig
rc.saveConfig = saveConfig
