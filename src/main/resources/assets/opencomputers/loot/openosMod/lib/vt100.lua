  ...
  for _,part in ipairs(parts) do
    ...
    if flip then
      if not window.flip then
        if window.gpu then
          local rgb, pal = window.gpu.getForeground()
          if math.floor(window.gpu.maxDepth()) == 1 then rgb = 0xFFFFFF pal = nil end
          local rgb, pal = bg(rgb, pal)
          if type(pal) == "string" then pal = nil end
          if not rgb then rgb = 0xFFFFFF end
          --require("superUtiles").logTo("/home/asdasd1.log", tostring(pal))
          --require("superUtiles").logTo("/home/asdasd2.log", tostring(rgb))
