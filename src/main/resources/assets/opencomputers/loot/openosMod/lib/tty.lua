...
function tty.keyboard()
  ...
  if window.keyboard and component.proxy(window.keyboard) then
  ...
  local screen = tty.screen()
  local proxy = component.proxy(screen or "")
  if proxy then
    local keyboard = proxy.getKeyboards()[1]
    tty.window.keyboard = keyboard
  end
  return tty.window.keyboard
end
