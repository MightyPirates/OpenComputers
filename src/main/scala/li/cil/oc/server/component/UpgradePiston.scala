...
class UpgradePiston(val container: Rotatable with Container) extends component.ManagedComponent {
  ...
  def push(context: Context, args: Arguments): Array[AnyRef] = {
    ...
    if (!container.world.isAirBlock(bx, by, bz) && node.tryChangeBuffer(-Settings.get.pistonCost) && tryExtend.invoke(Blocks.piston, container.world, x.underlying(), y.underlying(), z.underlying(), container.facing.ordinal.underlying()).asInstanceOf[Boolean]) {
