...
class UpgradeSign(val owner: Container with Rotatable) extends component.ManagedComponent {
  ...
  private def findSign = {
    ...
    owner.world.getBlockTileEntity(x, y, z) match {
      case sign: TileEntitySign => Option(sign)
      case _ => owner.world.getBlockTileEntity(x + owner.facing.offsetX, y + owner.facing.offsetY, z + owner.facing.offsetZ) match {
        case sign: TileEntitySign => Option(sign)
        case _ => None
      }
    }
