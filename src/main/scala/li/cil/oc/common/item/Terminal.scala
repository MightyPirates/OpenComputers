...
class Terminal(val parent: Delegator) extends traits.Delegate with CustomModel {
  ...
  def hasServer(stack: ItemStack) = stack.hasTagCompound && stack.getTagCompound.hasKey(Constants.namespace + "server")

  @SideOnly(Side.CLIENT)
  override def tooltipLines(stack: ItemStack, player: EntityPlayer, tooltip: util.List[String], advanced: Boolean) {
    super.tooltipLines(stack, player, tooltip, advanced)
    if (hasServer(stack)) {
      val server = stack.getTagCompound.getString(Constants.namespace + "server")
  ...
  private def modelLocationFromState(running: Boolean) = {
    new ModelResourceLocation(Constants.resourceDomain + ":" + Constants.ItemName.Terminal + (if (running) "_on" else "_off"), "inventory")
  }
