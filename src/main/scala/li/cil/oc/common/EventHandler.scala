...
object EventHandler {
  ...
  def playerLoggedIn(e: PlayerLoggedInEvent) {
    if (SideTracker.isServer) e.player match {
      ...
      case player: EntityPlayerMP =>
        ...
        if (!Settings.Power.ignorePower && Settings.Power.shouldIgnorePower()) {
  ...
  def onEntityJoinWorld(e: EntityJoinWorldEvent): Unit = {
    if (Settings.Misc.giveManualToNewPlayers && !e.getWorld.isRemote) e.getEntity match {
      case player: EntityPlayer if !player.isInstanceOf[FakePlayer] =>
        val persistedData = PlayerUtils.getPersistedData(player)
        if (!persistedData.getBoolean(Constants.namespace + "receivedManual")) {
          persistedData.setBoolean(Constants.namespace + "receivedManual", true)
  ...
  def onCrafting(e: ItemCraftedEvent) = {
    ...
    didRecraft = recraft(e, navigationUpgrade, stack => {
      ...
      Option(api.Driver.driverFor(e.crafting)) match {
        case Some(driver) => Option(new ItemStack(driver.dataTag(stack).getCompoundTag(Constants.namespace + "map")))
        case _ => None
      }
    }) || didRecraft
    ...
    e.player match {
      ...
      case player: EntityPlayerMP if player.getEntityWorld != null && !player.getEntityWorld.isRemote =>
        ...
        if (Settings.Misc.presentChance > 0 && !didRecraft && api.Items.get(e.crafting) != null &&
          e.player.getRNG.nextFloat() < Settings.Misc.presentChance && timeForPresents) {
          // Presents!
